...
/// Cost cells per provider — Today + month-to-date + all parsed history —
/// laid out horizontally to mirror `ChartsBlock`'s compact data row shape.
struct CostBlock: View {
    let color: Color
    let cost: ProviderCost
    let loading: Bool
    /// Identifies which provider this column is for so the VALUE multiplier
    /// can pick the right per-plan baseline (OpenClaw local logs vs Codex,
    ...
    var body: some View {
        HStack(spacing: centerWhenSingle ? 18 : 9) {
            CostTile(color: color, window: cost.today, loading: loading,
                     resetKind: .today, provider: provider,
                     centered: centerWhenSingle)
            CostTile(color: color, window: cost.month, loading: loading,
                     resetKind: .month, provider: provider,
                     centered: centerWhenSingle)
            CostTile(color: color, window: cost.total, loading: loading,
                     resetKind: .none, provider: provider,
                     centered: centerWhenSingle)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .top)
        .padding(.horizontal, centerWhenSingle ? 12 : 6)
    }
}
...
struct CostTile: View {
    enum ResetKind { case today, month, none }
    ...
    var body: some View {
        ...
        .accessibilityLabel(L10n.tr(window.label))
    ...
    /// plan: OpenClaw local logs, Codex Plus $20 / Pro $200.
    ...
    private var resetGlyph: String {
        ...
        switch resetKind {
        case .today: return "↻ " + CostBucketing.todayResetIn()
        case .month: return "↻ " + CostBucketing.monthResetIn()
        case .none: return ""
        }
    }
    ...
    private var resetGlyphSpoken: String {
        ...
        switch resetKind {
        case .today:
            return L10n.tr("Resets in %@", CostBucketing.todayResetIn())
        case .month:
            return L10n.tr("Resets in %@", CostBucketing.monthResetIn())
        case .none:
            return L10n.tr("All parsed local history")
        }
    }
}
