...
enum UsageFetcher {
    ...
    static func fetchCodex(rootPath: String? = nil) async -> AppUsage {
        guard let token = readCodexAccessToken(rootPath: rootPath) else {
    ...
    private static func readCodexAccessToken(rootPath: String? = nil) -> String? {
        let root = normalizedRoot(rootPath ?? "~/.codex", fallback: "~/.codex")
        let path = expandRoot(root).appendingPathComponent("auth.json").path
    ...
    static func fetchCodexResetCredits(rootPath: String? = nil) async -> CodexResetCredits? {
        guard let token = readCodexAccessToken(rootPath: rootPath) else { return nil }
    ...
    /// OpenClaw currently has no account quota endpoint equivalent to
    /// Codex/Anthropic usage APIs. The cost page is populated from local
    /// OpenClaw logs; the live 5h/week quota tiles stay passive instead of
    /// attempting Claude OAuth/keychain/network calls.
    static func fetchOpenClaw() async -> AppUsage {
        AppUsage(
            fiveHour: WindowUsage(usedPercent: 0, resetAt: nil, error: "local logs only"),
            weekly: WindowUsage(usedPercent: 0, resetAt: nil, error: "local logs only"),
            plan: nil
        )
    }
