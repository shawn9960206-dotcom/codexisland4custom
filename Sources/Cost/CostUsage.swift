...
/// Per-provider cost summary: today + month-to-date + all parsed local history
/// in calendar-local time.
struct ProviderCost {
    var today: CostWindow
    var month: CostWindow
    var total: CostWindow
    /// Per-model breakdown over the calendar-local current day. Used by the
    /// cost page's compact bar/table view.
    var todayByModel: [ModelUsageRow] = []
    /// Per-model breakdown over the calendar-local current month.
    var monthByModel: [ModelUsageRow] = []
    /// Per-model breakdown over all parsed local history.
    var totalByModel: [ModelUsageRow] = []
    ...
    /// the rate-limited 5h window used by the live tiles / usage page.
    var recentByModel: [ModelUsageRow] = []
    /// Per-model breakdown over the rolling last 7 days, sorted by tokens
    /// descending. Approximates the weekly window used by the live tiles / usage page.
    var weekByModel: [ModelUsageRow] = []
    /// Calendar-local daily dollar totals for the last 7 days, oldest first,
    /// with today included as the final bucket. Powers the cost-page trend.
    var last7DailyDollars: [Double] = []
    ...
    static let empty = ProviderCost(
        ...
        total: CostWindow(
            dollars: 0, tokens: 0, billableTokens: 0, series: [],
            label: L10n.tr("History"), error: nil, unknownModels: []
        ),
        todayByModel: [],
        monthByModel: [],
        totalByModel: [],
    ...
    static let dummy = ProviderCost(
        ...
        total: CostWindow(
            dollars: 428.0, tokens: 55_500_000, billableTokens: 5_550_000,
            series: [58, 104, 186, 271, 356, 428],
            label: L10n.tr("History"), error: nil, unknownModels: []
        ),
        todayByModel: [],
        monthByModel: [],
        ...
        recentByModel: [],
        weekByModel: [],
        last7DailyDollars: [6.0, 8.5, 12.0, 9.5, 14.0, 18.0, 11.25],
        dailyTokens: []
    )
}
