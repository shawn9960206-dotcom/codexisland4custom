...
enum CodexLogReader {
    static func scan(lookbackDays: Int = 30, rootPath: String? = nil) -> [TokenEvent] {
        ...
        LogParseCache.walk(
            roots: [sessionsRoot(rootPath: rootPath)],
            cutoff: cutoff,
    ...
    private static func sessionsRoot(rootPath: String? = nil) -> URL {
        if let rootPath {
            return expandRoot(normalizedRoot(rootPath, fallback: "~/.codex"))
                .appendingPathComponent("sessions", isDirectory: true)
        }
        if let codexHome = ProcessInfo.processInfo.environment["CODEX_HOME"], !codexHome.isEmpty {
            return expandRoot(codexHome).appendingPathComponent("sessions", isDirectory: true)
        }
