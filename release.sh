...
APP_NAME="codexisland4custom"
DIST="dist"
APP="$DIST/$APP_NAME.app"
DMG="$DIST/codexisland4custom-$VERSION.dmg"
CREATE_DMG_OUT="$DIST/codexisland4custom $VERSION.dmg"
...
# "codexisland4custom is damaged and can't be opened" failure mode that
...
create-dmg \
  --overwrite \
  --no-code-sign \
  --dmg-title "codexisland4custom $VERSION" \
  "$APP" \
  "$DIST"
...
if [[ -x "$SIGN_TOOL" && $have_key -eq 1 ]]; then
  ...
  RELEASE_URL="https://github.com/shawn9960206-dotcom/codexisland4custom/releases/download/v${VERSION}/$(basename "$DMG")"
...
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    <title>codexisland4custom</title>
    <link>https://github.com/shawn9960206-dotcom/codexisland4custom/releases/latest/download/appcast.xml</link>
    <description>Most recent codexisland4custom release.</description>
    ...
    <item>
      ...
      <sparkle:releaseNotesLink>https://github.com/shawn9960206-dotcom/codexisland4custom/releases/tag/v${VERSION}</sparkle:releaseNotesLink>
