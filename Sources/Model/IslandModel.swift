...
final class IslandModel: ObservableObject {
    ...
    /// Cost page now shows a 5-row model table plus a 7-day trend panel, so
    /// it needs a little more vertical room than the original two-tile page.
    private let costBaseContentHeight: CGFloat = 260
    ...
    private var expandedContentHeight: CGFloat {
        let baseHeight: CGFloat = {
            switch activeScreen {
            case .overview: return overviewBaseContentHeight
            case .cost:     return costBaseContentHeight
            case .usage:    return expandedBaseContentHeight
            }
        }()
