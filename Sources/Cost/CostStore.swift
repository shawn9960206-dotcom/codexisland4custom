...
/// session logs (OpenClaw + Codex CLI), aggregates today + month-to-date
...
final class CostStore: ObservableObject {
    ...
    /// Long local-log window used for the all-history total. Codex / OpenClaw
    /// sessions are local files, so a decade is effectively "everything this
    /// machine still has" without needing a new unbounded scan API.
    private static let historyScanDays = 3650
    ...
    func refresh() {
        ...
        let needsOpenCode = ((!claudeLoading && leftAgent == .codex)
            || (!codexLoading && rightAgent == .codex))
        let openCodeTask: Task<[TokenEvent], Never>? = needsOpenCode
            ? Task.detached(priority: .userInitiated) { OpenCodeLogReader.scan(lookbackDays: days) }
            : nil

        // Per-slot gate so a slow local-log scan doesn't block a fast one
        // on the next tick.
        if !claudeLoading {
            claudeLoading = true
    ...
    nonisolated private static func scanEvents(
        for agent: AgentKind,
        rootPath: String,
        days: Int,
        openCodeEvents: [TokenEvent]
    ) -> [TokenEvent] {
        switch agent {
        case .openClaw:
            return OpenClawLogReader.scan(lookbackDays: days, rootPath: rootPath)
        case .codex:
            return CodexLogReader.scan(lookbackDays: days, rootPath: rootPath)
                + openCodeEvents.filter { $0.provider == .codex }
        case .claudeCode:
            return ClaudeLogReader.scan(lookbackDays: days, rootPath: rootPath)
        }
    }
    ...
    private func loadDemoData() {
        ...
        self.claude = ProviderCost(
            ...
            total: CostWindow(
                dollars: 6120.40, tokens: 8_683_883_788, billableTokens: 868_388_378,
                series: [980.20, 2125.70, 3604.10, 4609.60, 6120.40],
                label: "History", error: nil, unknownModels: []
            ),
            last7DailyDollars: [86.4, 58.3, 63.6, 55.1, 53.8, 37.0, 146.61],
        ...
        self.codex = ProviderCost(
            ...
            total: CostWindow(
                dollars: 4864.20, tokens: 5_837_900_000, billableTokens: 1_167_580_000,
                series: [640.40, 1482.80, 2591.10, 3521.40, 4864.20],
                label: "History", error: nil, unknownModels: []
            ),
            last7DailyDollars: [51.7, 67.4, 84.6, 102.3, 118.8, 130.4, 136.50],
            dailyTokens: Self.demoDailyBuckets([
    ...
    private func restoreFromCache() {
        ...
        self.claude = ProviderCost(
            ...
            total: CostWindow(dollars: snap.claudeTotal, tokens: snap.claudeTotalTokens,
                              billableTokens: snap.claudeTotalBillable,
                              series: snap.claudeTotalSeries,
                              label: "History", error: nil,
                              unknownModels: snap.claudeTotalUnknown),
            last7DailyDollars: snap.claudeLast7DailyDollars,
        ...
        self.codex = ProviderCost(
            ...
            total: CostWindow(dollars: snap.codexTotal, tokens: snap.codexTotalTokens,
                              billableTokens: snap.codexTotalBillable,
                              series: snap.codexTotalSeries,
                              label: "History", error: nil,
                              unknownModels: snap.codexTotalUnknown),
            last7DailyDollars: snap.codexLast7DailyDollars,
