...
struct SettingsView: View {
    ...
    private var generalSection: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            SettingsRow(
                ...
                subtitle: launchStore.errorMessage ?? "Open codexisland4custom when you sign in."
            ) {
    ...
    private func showLanguageRestartPrompt() {
        let alert = NSAlert()
        alert.messageText = L10n.tr("Restart codexisland4custom to apply language?")
        alert.informativeText = L10n.tr("Your language change will take effect after codexisland4custom restarts.")
    ...
    private var providersSection: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            SettingsRow(
                ...
                HStack(spacing: 10) {
                    agentPicker(selection: $agentConfig.leftAgent)
            ...
            SettingsRow(
                ...
                HStack(spacing: 10) {
                    agentPicker(selection: $agentConfig.rightAgent)
            ...
            SettingsRow(
                title: "OpenClaw root",
                subtitle: "Default root: ~/.openclaw-autoclaw"
            ) {
                rootField($agentConfig.openClawRoot)
            }
            SettingsRow(
                title: "Codex root",
                subtitle: "Default root: ~/.codex"
            ) {
                rootField($agentConfig.codexRoot)
            }
            SettingsRow(
                title: "Claude Code root",
                subtitle: "Default root: ~/.claude"
            ) {
                rootField($agentConfig.claudeCodeRoot)
            }
        }
        .padding(.horizontal, 14)
        .padding(.top, 18)
    ...
    private func agentPicker(selection: Binding<AgentKind>) -> some View {
    ...
    private func rootField(_ text: Binding<String>) -> some View {
        TextField("", text: text)
            .textFieldStyle(.plain)
    ...
    /// dominates OpenClaw workflows. Both totals are computed every
