...
struct OverviewView: View {
    ...
    private var displayedOpenClawTokens: Int {
    ...
    private var summary: some View {
        HStack(alignment: .bottom, spacing: 18) {
            ...
            ProviderSplitRow(
                claudeTokens: displayedOpenClawTokens,
    ...
    private var summarySubline: String {
        ...
        case .claude: return L10n.tr("Mostly %@", agentConfig.displayName(for: .claude))
        case .codex:  return L10n.tr("Mostly %@", agentConfig.displayName(for: .codex))
    ...
    private var summaryAccessibilityLabel: String {
        if let selectedDay {
            return L10n.tr(
                ...
                agentConfig.displayName(for: .claude),
                Self.formatTokensSpoken(displayedOpenClawTokens),
                agentConfig.displayName(for: .codex),
...
private struct ContributionCell: View {
    ...
    private var dominanceLabel: String {
        switch day.dominantProvider {
        case .none:   return L10n.tr("No Activity")
        case .claude: return L10n.tr("Mostly %@", AgentConfigurationStore.shared.displayName(for: .claude))
        case .codex:  return L10n.tr("Mostly %@", AgentConfigurationStore.shared.displayName(for: .codex))
        case .mixed:  return L10n.tr("Mixed Use")
        }
    }
...
private struct DayDetailStrip: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            HStack(alignment: .center, spacing: 14) {
                ...
                if claudeVisible {
                    detailMetric(label: AgentConfigurationStore.shared.displayName(for: .claude).uppercased(), spokenLabel: AgentConfigurationStore.shared.displayName(for: .claude), value: day.claudeTokens, color: IslandColor.claude)
                }

                if codexVisible {
                    detailMetric(label: AgentConfigurationStore.shared.displayName(for: .codex).uppercased(), spokenLabel: AgentConfigurationStore.shared.displayName(for: .codex), value: day.codexTokens, color: IslandColor.codex)
                }
            }
        }
        .frame(height: 46)
    ...
    private var accessibilityLabel: String {
        L10n.tr(
            ...
            OverviewView.formatTokensSpoken(day.totalTokens),
            AgentConfigurationStore.shared.displayName(for: .claude),
            OverviewView.formatTokensSpoken(day.claudeTokens),
            AgentConfigurationStore.shared.displayName(for: .codex),
            ...
            HStack(spacing: 8) {
                if claudeVisible {
                    splitChip(
                        ...
                        label: AgentConfigurationStore.shared.displayName(for: .claude),
                        value: claudeTokens
                    )
                }
                if codexVisible {
                    splitChip(
                        ...
                        label: AgentConfigurationStore.shared.displayName(for: .codex),
                        value: codexTokens
                    )
                }
            }
