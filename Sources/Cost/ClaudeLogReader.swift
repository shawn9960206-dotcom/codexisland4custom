...
enum ClaudeLogReader {
    ...
    static func scan(lookbackDays: Int = 30, rootPath: String? = nil) -> [TokenEvent] {
        ...
        LogParseCache.walk(
            roots: projectRoots(rootPath: rootPath),
            cutoff: cutoff,
    ...
    private static func projectRoots(rootPath: String? = nil) -> [URL] {
        let fm = FileManager.default
        if let rootPath {
            let root = expandRoot(normalizedRoot(rootPath, fallback: "~/.claude"))
            let projects = root.lastPathComponent == "projects"
                ? root
                : root.appendingPathComponent("projects", isDirectory: true)
            return fm.fileExists(atPath: projects.path) ? [projects] : [projects]
        }
        if let env = ProcessInfo.processInfo.environment["CLAUDE_CONFIG_DIR"], !env.isEmpty {
            return env.split(separator: ",").map {
                expandRoot(String($0).trimmingCharacters(in: .whitespaces))
